      ...
      subroutine restrp(elimit, tnelem, tavailm, cerat, nparts, cfrac,
...
c        if ((availm(iel) .le. 1E-10) .and. (snfxmx .eq. 0.0) .and.
        if ((tavailm(1,iel) .le. 1E-4) .and. 
     &   (SNFXMX2f(idf) .eq. 0.0) .and.
     &      (aufert .eq. 0.0)) then
          goto 999
        endif
40    continue
...
!!... (favail) and the impact of root biomass (rimpct), adding storage.
      do 45 iel = 1, tnelem   !! check what if there are three elimit here?
        eavail(iel) = (tavailm(1,iel) * favail(iel) * rimpct) + 
     &                 forstg(j,iel)
45    continue
